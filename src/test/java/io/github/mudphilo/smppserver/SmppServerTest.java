...
import io.github.mudphilo.smppserver.*;
import io.github.mudphilo.smppserver.packet.EnquireLink;
import io.github.mudphilo.smppserver.packet.SmppResponse;
import io.github.mudphilo.smppserver.packet.SubmitSm;
import io.github.mudphilo.smppserver.packet.Unbind;
import io.github.mudphilo.smppserver.packet.DeliverSm;
...
public class SmppServerTest {
	...
	public void shouldCreateTranscieverSession() throws Exception {
		int PORT = 10000;
		shouldCreateSession(SmppSession.BindType.TRANSCIEVER, PORT);
	}
	
	@Test
	public void shouldCreateReceiverSession() throws Exception {
		int PORT = 10001;
		shouldCreateSession(SmppSession.BindType.RECEIVER, PORT);
	}
	...
	public void shoudlCreateTransmitterSession() throws Exception {
		...
		int PORT = 10002;
		shouldCreateSession(SmppSession.BindType.TRANSMITTER, PORT);
	}
	
	private void shouldCreateSession(SmppSession.BindType bindType, int PORT) throws Exception {
		...
		try {
			...
			if (SmppSession.BindType.RECEIVER.equals(bindType)) {

				chBindType = SmppBindType.RECEIVER;

			} else if (SmppSession.BindType.TRANSMITTER.equals(bindType)) {
	...
	public void shouldAcceptMultipleConnections() throws Exception {
		...
		try {
			...
			assertSessionsCreated(smppServer, 100, DEFAULT_TIMEOUT);
	...
	public void shouldSetCustomMessageId() throws Exception {
		...
		SmppServer smppServer = new SmppServer(PORT, new PacketProcessor() {
			...
			@Override
			public void processPacket(int sessionId,SmppRequest packet, ResponseSender responseSender) {
	...
	private void stopServer(SmppServer server, long timeout) throws InterruptedException {
